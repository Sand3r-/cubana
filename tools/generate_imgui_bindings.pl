...
# Original author: patrickriordan
# Source: https://github.com/patrickriordan/imgui_lua_bindings
# Modified to support more functions (default arguments, labels and etc.)
# By Michal 'Sand3r-' Gallus
#
    ...
    if ($line =~ m/$lineCaptureRegex/) {
      ...
      for (my $i = 0; $i < @args; $i++) {
        ...
        # float * x
          ...
          my $value = $3;
          if ($value) {
            push(@before, "DEFAULT_ARG(const char*, $name, $3)");
          } else {
            push(@before, "LABEL_ARG($name)");
          }
          push(@funcArgs, $name);
          # char *
        } elsif ($args[$i] =~ m/^\s*char\s*\*\s*([^\s=\[]*)$/) {
          my $name = $1;
          ...
          push(@funcArgs, $name);
        # ImVec2 
        } elsif ($args[$i] =~ m/^\s*ImVec2\s([a-zA-Z]*) *$/) {
          my $name = $1;
          push(@before, "IM_VEC_2_ARG($name)");
          push(@funcArgs, $name);
        ...
        # default arguments
        } elsif ($args[$i] =~ m/^\s*(.*)\s(.*)\s=\s(.*)$/) {
          my $type = $1;
          my $name = $2;
          my $value = $3;
          push(@before, "DEFAULT_ARG($type, $name, $value)");
          push(@funcArgs, $name);
        } elsif ($args[$i] =~ m/^\s*int\s*\*\s*([^\s=\[]*)$/) {
          ...
          my $ilosc = @functionsAlreadyAdded;
