...
const char * RunString(const char* szLua) {
  ...
  int iStatus = luaL_loadstring(g_ImguiBindingsState, szLua);
  if(iStatus) {
    return lua_tostring(g_ImguiBindingsState, -1);
    //fprintf(stderr, "Lua syntax error: %s\n", lua_tostring(g_ImguiBindingsState, -1));
    //return;
  }
#ifdef ENABLE_IM_LUA_END_STACK
  endStack.clear();
#endif
  iStatus = lua_pcall( g_ImguiBindingsState, 0, 0, 0 );
...
static void PushImguiEnums(lua_State* g_ImguiBindingsState, const char* tableName) {
  lua_pushstring(g_ImguiBindingsState, tableName);
...
#undef END_ENUM
#define START_ENUM(name) \
  lua_pushstring(g_ImguiBindingsState, #name); \
  lua_newtable(g_ImguiBindingsState); \
  { \
    int i = 1;
#define MAKE_ENUM(c_name,lua_name) \
  lua_pushstring(g_ImguiBindingsState, #lua_name); \
  lua_pushnumber(g_ImguiBindingsState, c_name); \
...
void LoadImguiBindings() {
  ...
  lua_setglobal(g_ImguiBindingsState, "imgui");
}
