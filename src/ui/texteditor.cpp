...
void TextEditor::HandleKeyboardInputs()
{
	...
	if (ImGui::IsWindowFocused())
	{
		...
		for (int i = 0; i < m_shortcuts.size(); i++) {
			...
			if ((ImGui::IsKeyPressed((ImGuiKey)sc1) || (sc1 == SDL_SCANCODE_RETURN && ImGui::IsKeyPressed((ImGuiKey)SDL_SCANCODE_KP_ENTER))) && ((sct.Key2 != -1 && ImGui::IsKeyPressed((ImGuiKey)SDL_GetScancodeFromKey(sct.Key2))) || sct.Key2 == -1)) {
...
void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
	...
	if (mFindOpened)
	{
		...
		for (int i = 0; i < m_shortcuts.size(); i++) {
			...
			if (ImGui::IsKeyPressed((ImGuiKey)sc1) && ((sct.Key2 != -1 && ImGui::IsKeyPressed((ImGuiKey)SDL_GetScancodeFromKey(sct.Key2))) || sct.Key2 == -1)) {
				if (((sct.Ctrl == (bool)0 && !ctrl) || (sct.Ctrl == (bool)1 && ctrl)) &&		// ctrl check
					((sct.Alt == (bool)0 && !alt) || (sct.Alt == (bool)1 && alt)) &&			// alt check
					((sct.Shift == (bool)0 && !shift) || (sct.Shift == (bool)1 && shift))) {// shift check
